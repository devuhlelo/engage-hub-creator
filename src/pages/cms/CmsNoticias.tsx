...
import { Save, Plus, Trash2, Edit, Newspaper } from "lucide-react";
...
const CmsNoticias = () => {
  ...
  useEffect(() => {
    setNoticias(getData("noticias", []));
  }, []);

  const save = (items: Noticia[]) => {
    setData("noticias", items);
    toast({ title: "Salvo!" });
  };
  ...
  const saveItem = () => {
    ...
    let updated: Noticia[];
    if (editingId === "new") {
      updated = [{ ...form, id: generateId() } as Noticia, ...noticias];
    } else {
      updated = noticias.map((n) => (n.id === editingId ? { ...n, ...form } as Noticia : n));
    }
    setNoticias(updated);
    save(updated);
    cancelEdit();
      ...
      {editingId && (
        <div className="cms-card mb-6 space-y-4 ring-2 ring-primary/20">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="cms-label">Categoria</label>
              <Input value={form.category || ""} onChange={(e) => setForm({ ...form, category: e.target.value })} placeholder="Ex: Política, Saúde..." />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="cms-label">Data</label>
              <Input type="date" value={form.date || ""} onChange={(e) => setForm({ ...form, date: e.target.value })} />
            </div>
            <div>
              <label className="cms-label">Link Externo (opcional)</label>
              <Input value={form.link || ""} onChange={(e) => setForm({ ...form, link: e.target.value })} placeholder="https://..." />
            </div>
          </div>
          <div>
            <label className="cms-label">Resumo</label>
            <Input value={form.resumo || ""} onChange={(e) => setForm({ ...form, resumo: e.target.value })} placeholder="Breve resumo para listagem" />
          </div>
          ...
          <div>
            ...
            <RichTextEditor value={form.content || ""} onChange={(v) => setForm({ ...form, content: v })} placeholder="Escreva a notícia completa..." />
          </div>
        ...
        {noticias.map((n) => (
          <div key={n.id} className="cms-card flex items-center gap-4">
            ...
            <div className="flex gap-1 shrink-0">
              ...
              <Button onClick={() => deleteItem(n.id)} variant="ghost" size="icon" className="text-destructive"><Trash2 className="h-4 w-4" /></Button>
            </div>
          </div>
        ))}
      </div>
