...
const defaultHome: HomeData = {
  ...
  propostasTitle: "Nossas Propostas",
  propostasSubtitle: "",
  junteSeTitle: "Junte-se a Nós",
  junteSeContent: "",
  junteSeButtonText: "Participar",
  junteSeButtonLink: "",
  footerText: "",
  footerLinks: [],
...
const CmsHome = () => {
  ...
  useEffect(() => {
    setFormData(getData("home", defaultHome));
  }, []);
  ...
  const removeBandeira = (id: string) => {
    update("bandeiras", data.bandeiras.filter((b) => b.id !== id));
  };
  ...
  const removeDiferencial = (id: string) => {
    update("diferenciais", data.diferenciais.filter((d) => d.id !== id));
  };
  ...
  return (
    <div>
      ...
      {activeBlock === "hero" && (
        <div className="cms-card space-y-4">
          <h2 className="cms-section-title flex items-center gap-2">
            <CheckCircle className="h-5 w-5 text-primary" /> Hero / Banner
          </h2>
          ...
          <div>
            <label className="cms-label">Título Principal</label>
            <Input value={data.heroTitle} onChange={(e) => update("heroTitle", e.target.value)} placeholder="Digite o título do hero" />
          </div>
          <div>
            <label className="cms-label">Subtítulo</label>
            <Textarea value={data.heroSubtitle} onChange={(e) => update("heroSubtitle", e.target.value)} placeholder="Subtítulo ou slogan" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="cms-label">Texto do Botão</label>
              <Input value={data.heroButtonText} onChange={(e) => update("heroButtonText", e.target.value)} />
            </div>
            <div>
              <label className="cms-label">Link do Botão</label>
              <Input value={data.heroButtonLink} onChange={(e) => update("heroButtonLink", e.target.value)} placeholder="https://..." />
            </div>
          </div>
        </div>
      )}
      ...
      {activeBlock === "sobre" && (
        <div className="cms-card space-y-4">
          <h2 className="cms-section-title flex items-center gap-2">
            <CheckCircle className="h-5 w-5 text-primary" /> Sobre
          </h2>
          <div>
            <label className="cms-label">Título da Seção</label>
            <Input value={data.sobreTitle} onChange={(e) => update("sobreTitle", e.target.value)} />
          </div>
          ...
          <div>
            ...
            <RichTextEditor value={data.sobreContent} onChange={(v) => update("sobreContent", v)} placeholder="Conte sobre você..." />
          </div>
        </div>
      )}
      ...
      {activeBlock === "bandeiras" && (
        <div className="cms-card space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="cms-section-title flex items-center gap-2 mb-0">
              <CheckCircle className="h-5 w-5 text-primary" /> Bandeiras
            </h2>
      ...
      {activeBlock === "diferenciais" && (
        <div className="cms-card space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="cms-section-title flex items-center gap-2 mb-0">
              <CheckCircle className="h-5 w-5 text-primary" /> Diferenciais
            </h2>
      ...
      {activeBlock === "propostas" && (
        <div className="cms-card space-y-4">
          <h2 className="cms-section-title flex items-center gap-2">
            <CheckCircle className="h-5 w-5 text-primary" /> Seção Propostas (Início)
          </h2>
          <div>
            <label className="cms-label">Título</label>
            <Input value={data.propostasTitle} onChange={(e) => update("propostasTitle", e.target.value)} />
          </div>
          <div>
            <label className="cms-label">Subtítulo</label>
            <Textarea value={data.propostasSubtitle} onChange={(e) => update("propostasSubtitle", e.target.value)} placeholder="Breve descrição das propostas" />
          </div>
      ...
      {activeBlock === "juntese" && (
        <div className="cms-card space-y-4">
          <h2 className="cms-section-title flex items-center gap-2">
            <CheckCircle className="h-5 w-5 text-primary" /> Junte-se a Nós
          </h2>
          <div>
            <label className="cms-label">Título</label>
            <Input value={data.junteSeTitle} onChange={(e) => update("junteSeTitle", e.target.value)} />
          </div>
          <div>
            <label className="cms-label">Conteúdo</label>
            <RichTextEditor value={data.junteSeContent} onChange={(v) => update("junteSeContent", v)} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="cms-label">Texto do Botão</label>
              <Input value={data.junteSeButtonText} onChange={(e) => update("junteSeButtonText", e.target.value)} />
            </div>
            <div>
              <label className="cms-label">Link do Botão</label>
              <Input value={data.junteSeButtonLink} onChange={(e) => update("junteSeButtonLink", e.target.value)} />
            </div>
          </div>
        </div>
      )}
      ...
      {activeBlock === "footer" && (
        <div className="cms-card space-y-4">
          <h2 className="cms-section-title flex items-center gap-2">
            <CheckCircle className="h-5 w-5 text-primary" /> Footer
          </h2>
          <div>
            <label className="cms-label">Texto do Footer</label>
            <Textarea value={data.footerText} onChange={(e) => update("footerText", e.target.value)} placeholder="© 2024 Todos os direitos reservados" />
          </div>
