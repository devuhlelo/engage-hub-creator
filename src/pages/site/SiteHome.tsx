    ...
    <div>
      <section className="relative min-h-[80vh] flex items-center justify-center text-center text-white overflow-hidden"
        ...
        <div className="relative z-10 px-6 max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/site/propostas" className="inline-flex items-center justify-center gap-2 px-8 py-4 font-semibold text-white shadow-xl hover:scale-105 transition-all" style={{ background: theme.accentColor, borderRadius: btnRadius }}>
              {home?.heroButtonText || "Ver Propostas"} <ArrowRight className="h-5 w-5" />
            </Link>
            <Link to="/site/biografia" className="inline-flex items-center justify-center gap-2 px-8 py-4 font-semibold border-2 border-white/30 hover:bg-white/10 transition-all" style={{ borderRadius: btnRadius }}>
              Conheça a História
            </Link>
          </div>
        </div>
        <div className="absolute bottom-0 left-0 right-0">
          <svg viewBox="0 0 1440 80" fill="none"><path d="M0 80L60 68C120 56 240 32 360 24C480 16 600 24 720 36C840 48 960 64 1080 64C1200 64 1320 48 1380 40L1440 32V80H0Z" fill={theme.bodyBg} /></svg>
        </div>
      </section>
      ...
      <section className="py-24 px-6">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          {home?.sobreImage ? (
            <div className="relative"><img src={home.sobreImage} alt="" className="rounded-2xl shadow-2xl w-full" /></div>
          ) : (
          ...
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-bold mt-2 mb-6 leading-tight">{home?.sobreTitle || "Quem Somos"}</h2>
            <div className="prose max-w-none opacity-75 leading-relaxed" dangerouslySetInnerHTML={{ __html: home?.sobreContent || "<p>Configure esta seção no painel CMS.</p>" }} />
            <Link to="/site/biografia" className="inline-flex items-center gap-2 mt-6 font-semibold text-sm hover:gap-3 transition-all" style={{ color: theme.primaryColor }}>
      ...
      {propostas.length > 0 && (
        <section className="py-24 px-6 relative overflow-hidden" style={{ background: theme.secondaryColor, color: "#fff" }}>
          <div className="max-w-6xl mx-auto relative z-10">
            <div className="text-center mb-16">
              <span className="text-sm font-semibold uppercase tracking-wider" style={{ color: theme.accentColor }}>{home?.propostasTitle || "Propostas"}</span>
              <h2 className="text-3xl md:text-4xl font-bold mt-2">{home?.propostasSubtitle || "Conheça nossas principais propostas"}</h2>
            </div>
      ...
      <section className="py-24 px-6">
        <div className="max-w-4xl mx-auto text-center p-12 md:p-16 relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${theme.primaryColor}, ${theme.primaryColor}dd)`, borderRadius: "24px", color: "#fff" }}>
          ...
          <h2 className="text-3xl md:text-4xl font-bold mb-4">{home?.junteSeTitle || "Junte-se a Nós"}</h2>
          <div className="opacity-80 mb-8 max-w-xl mx-auto leading-relaxed" dangerouslySetInnerHTML={{ __html: home?.junteSeContent || "<p>Faça parte dessa transformação.</p>" }} />
          <Link to="/site/contato" className="inline-flex items-center gap-2 px-8 py-4 font-semibold bg-white shadow-xl hover:scale-105 transition-all" style={{ color: theme.primaryColor, borderRadius: btnRadius }}>
